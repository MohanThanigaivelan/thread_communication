...
# Main thread voluntarily goes to sleep when it needs the result. Background thread wakes up the main thread
...
class PostgresPipeline
  def initialize
    @lock = Monitor.new
    @condition_variable = @lock.new_cond
  end
    ...
    Thread.new do
      ...
      loop do
        ...
        @lock.synchronize do
          ...
          if @required_future_result == future
            @condition_variable.signal
            @required_future_result = nil
          end
        end
      end
    end
  ...
  def process_result(req_future)
    @lock.synchronize do
      @required_future_result = req_future
...
# CPU is not utilised while waiting for results as it is on sleep stete